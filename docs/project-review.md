...
13. ◐ token 迁出 localStorage——前端兼容层已就绪（2026-07-05）：新增 `src/services/session.ts` 双模会话（legacy token / httpOnly cookie），模式由登录响应决定——服务端返回 token 走旧路径；返回无 token 的 2xx 即视为已建立 cookie 会话（`markCookieSession`）。cookie 模式下请求自动带 `credentials: 'include'`（token 模式不带，避免影响现有通配 CORS 部署）；登出时 best-effort 调 `POST /user/logout` 清除 cookie（容忍旧服务端 404）。路由守卫与 useUserInfo 改用 `hasSession()`。request.ts re-export 旧接口面，调用方无感。**服务端就绪后无需再改前端**，只需登录接口改为 Set-Cookie + 不返回 token；届时另需确认 CORS `Access-Control-Allow-Credentials` 与 CSRF 防护。（§2.3）
