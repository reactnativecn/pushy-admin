...
12. ✅ 基本完成（2026-07-05）：`vanilla-jsoneditor` 改 `React.lazy` 按需加载（已从应用管理页 chunk 拆出独立异步 chunk，~275K）；`staleTime` 5s → 30s（切标签页不再重复请求，实时页有自己的 refetchInterval 不受影响）；**xlsx 已替换为 `write-excel-file`**（导出 chunk 275.5K/92.9K gzip → 69.5K/19.1K gzip，约 -80%，同时移除停更且带 CVE 的 npm 版 SheetJS；保留 .xlsx 格式与列宽）。charts 按需子包未做（路由级已 lazy，收益有限）。（§4.2、§4.3）
